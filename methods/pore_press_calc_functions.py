...
class Diffusion_with_Source_and_Gravity(PDEBase):
    ...
    def get_pore_ini_field(self):
        '''
        method to obtain pore pressure initial state. Just for convenience here.
        '''
...
def test_wells(eq, params, k_list=None):
    ...
    with h5py.File('model_injectivity_testing.h5', 'w') as targ: # saving 
...
def get_q_factors(perm, p0_field, params):
    ...
    with h5py.File('model_injectivity_testing.h5', 'r') as f:  
        p_k = f["p_k"][:]
        k_list = f["k_list"][:]
